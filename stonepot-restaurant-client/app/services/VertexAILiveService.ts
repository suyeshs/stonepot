...
export class VertexAILiveService {
        ...
        this.ws.onmessage = (event) => {
          ...
          try {
            ...
            if (message.type === 'session_started') {
              ...
              console.log('[VertexAILive] Session started and ready:', this.sessionId);
              resolve();
            }

            if (message.type === 'session_ready') {
              console.log('[VertexAILive] Session ready confirmation');
              this.sessionReady = true;
              if (!this.sessionId) {
                resolve();
              }
  ...
  /**
   * Interrupt AI response (user spoke while AI was speaking)
   */
  interrupt(): void {
    console.log('[VertexAILive] Sending interruption signal');
   ...
   */
  isActive(): boolean {
    return this.isConnected && this.sessionReady && this.ws !== null && this.ws.readyState === WebSocket.OPEN;
  }
