/**
 * Guardrails Service for Stonepot Restaurant
...
class Guardrails {
  ...
  async validateInput(input, sessionId, language = 'en') {
    try {
      ...
      console.error('[Guardrails] Input validation error:', error);
      return {
        passed: false,
  ...
  async validateResponse(response, input, conversationHistory = []) {
    try {
      ...
      if (conversationHistory.length > this.config.maxHistoryLength) {
        console.warn('[Guardrails] Conversation history too long', {
          historyLength: conversationHistory.length
        });
      }

      console.log('[Guardrails] Response validation passed', {
      ...
      console.error('[Guardrails] Response validation error:', error);
      return {
        passed: false,
        reason: 'Validation error',
        action: 'block'
