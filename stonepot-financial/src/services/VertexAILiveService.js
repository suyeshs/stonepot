...
class VertexAILiveService {
  ...
  async createSession(sessionId, config = {}) {
    ...
    try {
      ...
      logger.info('[VertexAILive] createSession called', { sessionId, language, userId });
      ...
      logger.info('[VertexAILive] Connecting to Vertex AI', {
        sessionId,
        wsUrl,
        model: this.model,
        location: this.location
      ...
      // CRITICAL: Register ALL event listeners IMMEDIATELY before any async operations
      // Log all WebSocket events for debugging
      ws.on('unexpected-response', (request, response) => {
        logger.error('[VertexAILive] WebSocket unexpected response', {
          sessionId,
      ...
      ws.on('upgrade', (response) => {
        logger.info('[VertexAILive] WebSocket upgrade response received', {
          sessionId,
          statusCode: response.statusCode
        });
      });

      // Wait for connection to open BEFORE doing any other async operations
      await new Promise((resolve, reject) => {
        const timeout = setTimeout(() => {
          logger.error('[VertexAILive] WebSocket connection timeout', { sessionId });
          ws.terminate(); // Force close the hung connection
      ...
      // NOW that connection is established, load previous conversation history if userId provided
      let previousContext = '';
      if (userId) {
      ...
      await new Promise((resolve, reject) => {
        ...
        const setupTimeout = setTimeout(() => {
          logger.error('[VertexAILive] Setup timeout - Vertex AI did not respond with setupComplete', {
            sessionId,
            timeoutSeconds: 15
          });
